...
async function handleLogin(event) {
    ...
    try {
        ...
        if (data.success) {
            ...
            // IMPORTANTE: Aguarda antes de mostrar o app
            setTimeout(() => {
                showApp();
            }, 50);
...
// Show main app - FUNÇÃO CORRIGIDA!
function showApp() {
    ...
    // IMPORTANTE: Adiciona classe 'active' para iOS e força display
    ...
    // Força atualização da viewport no mobile
    if (window.innerWidth < 768) {
        setTimeout(() => {
            window.scrollTo(0, 0);
            setVH();
...
function handleLogout() {
    ...
    // Remove classe active do container
    const appContainer = document.getElementById('appContainer');
    if (appContainer) {
...
function renderMessages() {
    ...
    currentChat.messages.forEach((msg, index) => {
        ...
        // Substitui marcador do gráfico se existir
        ...
        // Substitui marcador do artefato se existir
        ...
        // Renderiza o gráfico após inserir o HTML
        if (msg.chart) {
            const chartId = `chart-${currentChatId}-${index}`;
            setTimeout(() => {
